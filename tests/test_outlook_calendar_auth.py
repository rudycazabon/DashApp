"""Tests for tools/outlook_calendar/auth.py — MSAL token management."""
...
from tools.outlook_calendar.auth import (
    CREDENTIALS_PATH,
    TOKEN_PATH,
    get_access_token,
)
...
def test_token_path_under_app_dir() -> None:
    """TOKEN_PATH should reside inside APP_DIR with calendar-specific name."""
    assert TOKEN_PATH.parent == APP_DIR
    assert TOKEN_PATH.name == "outlook_calendar_token.json"
...
def test_get_access_token_raises_when_credentials_file_missing(
    ...
    with patch("tools.outlook_calendar.auth.CREDENTIALS_PATH", missing):
        with pytest.raises(
...
def test_get_access_token_uses_silent_flow_when_cache_has_account(
    ...
    with (
        patch("tools.outlook_calendar.auth.CREDENTIALS_PATH", fake_creds_path),
        patch(
            "tools.outlook_calendar.auth.TOKEN_PATH",
            tmp_path / "outlook_calendar_token.json",
        ),
        patch(
            "tools.outlook_calendar.auth.msal.SerializableTokenCache",
            return_value=mock_cache,
        ),
        patch(
            "tools.outlook_calendar.auth.msal.PublicClientApplication",
    ...
    mock_app.acquire_token_silent.assert_called_once_with(
        ["Calendars.Read"], account=mock_account
    )
...
def test_get_access_token_uses_interactive_flow_when_no_cache(
    ...
    with (
        ...
        patch("tools.outlook_calendar.auth.APP_DIR", tmp_path),
        patch(
            "tools.outlook_calendar.auth.msal.SerializableTokenCache",
            return_value=mock_cache,
        ),
    ...
    mock_app.acquire_token_interactive.assert_called_once_with(
        scopes=["Calendars.Read"]
    )
    saved = (tmp_path / "outlook_calendar_token.json").read_text()
