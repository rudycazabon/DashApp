"""Tests for tools/jira/auth.py — Basic Auth session management."""
...
def test_get_auth_raises_when_credentials_file_missing(tmp_path: Path) -> None:
    """FileNotFoundError is raised when jira_credentials.json is absent."""
    missing = tmp_path / "jira_credentials.json"
    with patch("tools.jira.auth.CREDENTIALS_PATH", missing):
        with pytest.raises(FileNotFoundError, match="jira_credentials.json not found"):
            get_auth()
...
def test_get_auth_returns_session_with_basic_auth(tmp_path: Path) -> None:
    ...
    creds_path = tmp_path / "jira_credentials.json"
    creds_path.write_text(
        '{"url": "https://example.atlassian.net", '
        '"email": "user@example.com", "api_token": "token123"}'
    )
    with patch("tools.jira.auth.CREDENTIALS_PATH", creds_path):
        session, _ = get_auth()
...
def test_get_auth_returns_correct_base_url(tmp_path: Path) -> None:
    """get_auth returns the URL from the credentials file."""
    creds_path = tmp_path / "jira_credentials.json"
    creds_path.write_text(
