...
class CalendarWidget(Widget):
    ...
    DEFAULT_CSS = """
    CalendarWidget { height: 1fr; }
    CalendarWidget #status-bar {
        height: 3;
        padding: 0 1;
        background: $surface;
        border-bottom: tall $panel;
    }
    CalendarWidget #dot { width: 3; content-align: center middle; }
    CalendarWidget #status-text { width: 1fr; content-align: left middle; }
    CalendarWidget #progress { width: 30; }
    CalendarWidget #scroll { height: 1fr; }
    """
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="status-bar"):
            ...
            yield Static("Connecting to Calendar…", id="status-text")
    ...
    def _load(self) -> None:
        """Fetch calendar events in a worker thread; update UI on the main thread."""
    ...
    def _populate(self, events: list[dict[str, str]]) -> None:
        """Mount Collapsible entries for each event; update status and hide progress."""
        try:
        ...
        for event in events:
            start = event["start"]
            # Format HH:MM for timed events, or "All day" for date-only events
            if "T" in start:
                # dateTime format: 2026-02-19T09:00:00+00:00 → 09:00
...
class Tool(BaseTool):
    """DashApp plugin that shows today's Google Calendar events."""
    ...
    def build_widget(self) -> Widget:
        """Return the Calendar widget for the tab."""
